...
function login($username, $password, $link) {
    ...
    if ($stmt = $link->prepare("SELECT id, email, password, salt 
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $link) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    $now = time();
                    $link->query("INSERT INTO login_attempts(user_id, time)
                                    VALUES ('$user_id', '$now')");
                    return false;
                }
            }
...
function checkbrute($user_id, $link) {
    ...
    if ($stmt = $link->prepare("SELECT time 
                             FROM login_attempts 
                             WHERE user_id = ? 
...
function login_check($link) {
    ...
    if (isset($_SESSION['user_id'], 
        ...
        if ($stmt = $link->prepare("SELECT password 
                                      FROM spinout_employees 
                                      WHERE id = ? LIMIT 1")) {
